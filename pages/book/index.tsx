...
const Book = () => {
  ...
  const handleBookEdit = async (data: any) => {
    try {
      ...
      const index = books.findIndex((book: any) => book?.id === data?.id);
  ...
  const handleUserDelete = async (deletedBook: any) => {
    try {
      ...
      setBooks((prevBooks) =>
        prevBooks.filter((book: any) => book.id !== deletedBook.id)
      );
    } catch (error) {
  ...
  return (
    <>
      ...
      <div className="text-md text-gray-600 my-4">
        You can manage your books here by adding, editing, or deleting them.
      </div>
